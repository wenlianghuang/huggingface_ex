...
with gr.Blocks() as demo:
    gr.Markdown(
        """
  # Retrieval Augmented Generation with Phi-3: Question Answering demo
  ### This demo uses the Phi-3 language model and Retrieval Augmented Generation (RAG). It allows you to upload a txt file and ask the model questions related to the content of that file.
  ### If you don't have one, there is a txt file already loaded, the new Oppenheimer movie's entire wikipedia page. The movie came out very recently in July, 2023, so the Phi-3 model is not aware of it.
  The context size of the Phi-3 model is 2048 tokens, so even this medium size wikipedia page (11.5k tokens) does not fit in the context window.
